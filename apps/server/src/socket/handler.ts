...
export function createSocketServer(httpServer: HttpServer): SocketIOServer {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-game", ({ gameId, playerInfo }: { gameId: string; playerInfo: { animal: string; color: string } }) => {
      roundManager.joinGame(socket, gameId, playerInfo);
    });

    socket.on("guess-submit", ({ gameId, roundId, guess }: { gameId: string; roundId: string; guess: string }) => {
